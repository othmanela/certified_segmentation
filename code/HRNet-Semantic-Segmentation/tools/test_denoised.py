...
def sample(image_np01, size, n, sigma, model, config, test_dataset, do_tqdm=True, unscaled=False):
    ...
    with torch.no_grad():
        while (remaining) > 0:
            ...
            pred = test_dataset.multi_scale_inference_denoisedbatch(
                model,
                cnt, sigma,
                image_np01,
                dd_model,
...
def testval(args, config, test_dataset, testloader, model):
    ...
    log = Log(config.DATASET.DATASET + args.name, str(args.sigma))
    log.write(str(args) + "\n")
    ...
    with torch.no_grad():
        for index, batch in itt:
            ...
            t1 = time.time()
            save_segmentation(classes_baseline, test_dataset, palette, log.folder() / f"{index}_baseline.png", abstain,
                              abstain_mapping)
            ...
            for k in range(len(args.n)):
                ...
                n = args.n[k]
                t0 = time.time()
                s = sample(image_np01, size, args.n0 + n, args.sigma, model, config, test_dataset,
                ...
                for c in corr:
                    classes_certify, radius, timings = certify(num_classes - 1, s, args.n0, n, args.sigma, tau,
                                                               args.alpha, abstain=abstain, parallel=True, correction=c)
                    classes_certify = np.reshape(classes_certify, (1, *s_shape[1:]))
                    if args.unscaled:
                        classes_certify = cv2.resize(np.transpose(classes_certify, (1, 2, 0)).astype(np.float32),
                                                     dsize=(size[-2], size[-1]), interpolation=cv2.INTER_NEAREST)
                        classes_certify = np.reshape(classes_certify, (1, size[-2], size[-1])).astype(np.int64)
                    save_segmentation(classes_certify, test_dataset, palette,
                                      log.folder() / f"{index}_certify_{c}_{n}_{tau}.png", abstain, abstain_mapping)
